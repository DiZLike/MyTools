"""
AudioUNet — MP3 RESTORER v2 (Hi-Res Direct)
...
CACHE_DIR = DATASET_DIR / "cache_mp3_128_v2"
CHECKPOINT_DIR = Path("checkpoints_mp3_128_v2")
REPORTS_DIR = Path("reports_mp3_128_v2")
...
LAMBDA_MSE = 10.0      # Высокий вес на точность восстановления
LAMBDA_ADV = 1.0
LAMBDA_FM = 2.0
LAMBDA_TEX = 0.5
...
class AugmentedSubsetV2(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if self.augment:
            x, y = x.float(), y.float()
            import torchaudio
            if torch.rand(1) > 0.5: x = torchaudio.transforms.FrequencyMasking(FREQ_MASK_SIZE)(x)
            if torch.rand(1) > 0.5: x = torchaudio.transforms.TimeMasking(TIME_MASK_SIZE)(x)
            x, y = x.half(), y.half()
        return x, y
...
def train_epoch(generator, discriminator, loader, opt_g, opt_d, device):
    ...
    for x, y_real in pbar:
        x = x.to(device, dtype=torch.float32)
        y_real = y_real.to(device, dtype=torch.float32)
        
        # --- Дискриминатор ---
        opt_d.zero_grad()
        y_fake = generator(x).detach()
        real_preds = discriminator(y_real)
        fake_preds = discriminator(y_fake)
        d_loss = sum(F.mse_loss(rp, torch.ones_like(rp)) + F.mse_loss(fp, torch.zeros_like(fp))
        ...
        real_preds = discriminator(y_real)
        fm_loss = sum(F.l1_loss(fp.detach(), rp.detach()) for fp, rp in zip(fake_preds, real_preds))
        
        mse_loss = F.l1_loss(y_fake, y_real)
        tex_loss = ((y_fake.std(dim=3) - y_real.std(dim=3)).abs()).mean()
        
        g_loss = LAMBDA_MSE * mse_loss + LAMBDA_ADV * adv_loss + LAMBDA_FM * fm_loss + LAMBDA_TEX * tex_loss
        g_loss.backward()
        torch.nn.utils.clip_grad_norm_(generator.parameters(), GRADIENT_CLIP)
        opt_g.step()
        
        total_g_loss += g_loss.item(); total_d_loss += d_loss.item()
        pbar.set_postfix(g=f'{total_g_loss/(pbar.n+1):.3f}', d=f'{total_d_loss/(pbar.n+1):.3f}',
...
def validate(generator, loader, vis_loader, device):
    ...
    for i, (x, y) in enumerate(pbar):
        ...
        total_mse += mse.item()
        pbar.set_postfix(mse=f'{total_mse/(i+1):.4f}')
    
    vis_data = next(iter(DataLoader(vis_loader, batch_size=min(BATCH_SIZE, len(vis_loader)), shuffle=False)))
    x_vis, y_vis = vis_data
    x_vis = x_vis.to(device, dtype=torch.float32)
    pred_vis = generator(x_vis).cpu()
    
    if pred_vis.shape[0] >= 3:
        err = torch.abs(pred_vis - y_vis).mean(dim=[1,2,3])
        idx = err.argsort()
        sample_data = (
            x_vis[idx[[0, len(idx)//2, -1]]].cpu(),
            y_vis[idx[[0, len(idx)//2, -1]]].cpu(),
            pred_vis[idx[[0, len(idx)//2, -1]]].cpu()
        )
    
    return total_mse / len(loader), sample_data
...
def save_checkpoint(epoch, g, d, opt_g, opt_d, best_loss, path):
    torch.save({'epoch': epoch, 'g_state_dict': g.state_dict(), 'd_state_dict': d.state_dict(),
                'opt_g': opt_g.state_dict(), 'opt_d': opt_d.state_dict(), 'best_loss': best_loss}, path)
...
def plot_dashboard(epoch, g_losses, d_losses, val_mses, lr_history, sample_data, save_dir):
    ...
    inp, full_high, pred = sample_data
    num = min(inp.shape[0], 2)
    fig = plt.figure(figsize=(24, 14))
    
    for i, (label, data) in enumerate([
        ('G Loss', g_losses), ('D Loss', d_losses), ('Val MSE', val_mses), ('Info', None)
    ]):
        ax = plt.subplot(3, 4, i+1)
        if i < 3: ax.plot(data, 'o-', markersize=3); ax.set_title(label); ax.grid(alpha=0.3)
        else:
            ax.axis('off')
            ax.text(0.1, 0.9, f"Epoch: {epoch+1}\nG: {g_losses[-1]:.3f}\nD: {d_losses[-1]:.3f}\n"
                    f"Val: {val_mses[-1]:.4f}\nMP3 128 → Lossless",
    ...
    names = ['BEST', 'MEDIAN']
    for s in range(num):
        inp_np = inp[s, 0].numpy()
        full_high_np = full_high[s, 0].numpy()
        pred_np = pred[s, 0].numpy()
        
        min_t = min(inp_np.shape[1], full_high_np.shape[1], pred_np.shape[1])
        inp_np = inp_np[:, :min_t]
        full_high_np = full_high_np[:, :min_t]
        pred_np = pred_np[:, :min_t]
        
        diff_np = np.abs(pred_np - full_high_np)
        
        base_idx = 4 * (s + 1)
        for col, (data, title, cmap, vmin, vmax) in enumerate([
            (inp_np, f'[{names[s]}] MP3 Input', 'viridis', 0, 1),
            (full_high_np, 'Target Lossless', 'viridis', 0, 1),
            (pred_np, 'Predicted', 'viridis', 0, 1),
            (diff_np, 'Error', 'hot', 0, 0.5)
        ]):
            ax = plt.subplot(3, 4, base_idx + col + 1)
...
if __name__ == "__main__":
    CHECKPOINT_DIR.mkdir(exist_ok=True); REPORTS_DIR.mkdir(exist_ok=True)
    
    print(f"╔══════════════════════════════════════╗")
    print(f"║  MP3 Restorer v2 — Direct Hi-Res   ║")
    print(f"╚══════════════════════════════════════╝")
    ...
    if os.path.exists(CHECKPOINT_FILE):
        start_epoch, best_loss = load_checkpoint(CHECKPOINT_FILE, generator, discriminator, opt_g, opt_d, DEVICE)
        print(f"→ Загрузка: эпоха {start_epoch}, best MSE {best_loss:.4f}\n")
    else:
    ...
    for epoch in range(start_epoch, EPOCHS):
        ...
        g_loss, d_loss = train_epoch(generator, discriminator, train_loader, opt_g, opt_d, DEVICE)
        val_mse, sample_data = validate(generator, val_loader, vis_dataset, DEVICE)
        sched_g.step(); sched_d.step()
        
        g_losses.append(g_loss); d_losses.append(d_loss); val_mses.append(val_mse)
        lr_history.append(opt_g.param_groups[0]['lr'])
        
        print(f"   G: {g_loss:.3f} | D: {d_loss:.3f} | Val MSE: {val_mse:.4f} | LR: {opt_g.param_groups[0]['lr']:.2e}")
        
        if val_mse < best_loss:
            best_loss = val_mse; no_improve = 0
            torch.save(generator.state_dict(), BEST_MODEL_FILE)
            print(f"   ✓ Лучшая модель (MSE={best_loss:.4f})")
        else:
            no_improve += 1
        ...
        plot_dashboard(epoch, g_losses, d_losses, val_mses, lr_history, sample_data, REPORTS_DIR)
        
        if no_improve >= EARLY_STOP_PATIENCE:
            print(f"\n⏹️  Ранняя остановка"); break
    
    print(f"\n{'='*60}\n🏁 Готово! Best Val MSE: {best_loss:.4f}")
    print(f"   Generator: {BEST_MODEL_FILE}")